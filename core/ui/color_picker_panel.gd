...
func setup(settings = null, settings_key: String = "") -> void:
    _settings = settings
    if settings_key != "":
...
func _draw_alpha_slider():
    ...
    var checker_size = 4.0
    var checker_step = int(checker_size)
    for x in range(0, int(_alpha_slider.size.x), checker_step):
        for y in range(0, int(_alpha_slider.size.y), checker_step):
            var cell_x = int(float(x) / checker_size)
            var cell_y = int(float(y) / checker_size)
...
func set_color(c: Color, emit_color_signal: bool = false):
...
func _update_all_ui(emit_color_signal: bool = false):
    _updating_ui = true
